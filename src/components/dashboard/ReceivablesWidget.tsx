...
import { Loader2, RefreshCw, HandCoins, AlertCircle } from "lucide-react";
...
export function ReceivablesWidget({ tenantId, tenantName }: { tenantId: string; tenantName: string }) {
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Unpaid customer invoices · as of {data?.asOf ?? "—"}
          </p>
        </div>
        ...
        data.totalOutstanding === 0 ? (
          ...
          <>
            <div className="mt-6 grid grid-cols-3 gap-3">
              ...
              />
              <Kpi label="Open Invoices" value={String(data.invoiceCount)} />
            </div>
...
function Kpi({ label, value, tone }: { label: string; value: string; tone?: "negative" }) {
