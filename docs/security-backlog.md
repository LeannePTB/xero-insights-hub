...
1. **`SECURITY DEFINER` functions executable by `authenticated`** — partially addressed, still open. Rounds 1 and 2 fixed the specific functions triaged so far (`_user_id` and `_firm_id` caller-identity oracles, `shares_firm_with`, `xero_tenant_already_linked`, `xero_missing_scopes`) and moved several write paths behind membership-gated definer functions. Round 2 changed no functions at all. The linter still reports 24 definer functions callable by signed-in users, and the majority have not been read line by line. Treat this item as open until every one is triaged.
