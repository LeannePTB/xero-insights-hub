@@
 - 20 Sep 2026 — CLOSED: `true_breakeven` was retired from the card catalogue and all stored client ticks in one fail-loud production migration. Classification: SECURITY-RELEVANT because it rewrote existing client card settings. Root cause of the two earlier reported “failures”: no removal statement had been submitted at all; production migration history, audit history and unchanged row timestamps proved there was no successful no-op and no revert. The correction first asserted the exact production before shape (ten affected clients: nine DRTABT organisation clients at 16 cards, Positive Traction at 14), removed the key from `app_private.card_group_cards`, every `client_cards` list and every organisation default, and aborted unless exactly ten clients/ticks changed and zero remnants remained. Audit action `card_catalogue_card_removed` records all ten before/after pairs. Verified after: DRTABT's nine 16 → 15, Positive Traction 14 → 13, Bangkok on King 6 → 6, Autotek New South Wales 6 → 6; zero client/default ticks remain. `client_true_breakeven_inputs`, its AAL2 server functions and the implementation are retained for a future rebuild; the table contained zero rows at retirement. The reverse catalogue guard now fails if a removed key survives in either stored tick source.
 - 21 Sep 2026 — CLOSED: the `true_breakeven` tick now draws something. Classification: NOT security-relevant — presentation and an existing server function only. No policy, grant, definer function, role, entitlement, purchase, trial, Xero credential or server function signature changed; `getTrueBreakevenInputs` / `upsertTrueBreakevenInputs` already existed with `requireAal2` and the existing RLS on `public.client_true_breakeven_inputs` (owner/firm-access to write, `has_client_read_access` to read, `mfa_aal2_required`) is unchanged, so a viewer can read and not write. Defect: the card key was offered and ticked for every advisory client while nothing rendered it and no screen existed to enter the figures — the table was empty for all 14 clients, so no figures were at risk. Change: new pure `src/components/dashboard/true-breakeven-figures.ts` (monthly basis, matching the rest of the card) with 6 tests; new `src/components/dashboard/TrueBreakevenSection.tsx` rendered inside the Break-Even card only when `true_breakeven` is ticked; a "What you really need to bring in" section in client settings, shown only when that card key is allowed, writing through the existing audited server function. Wording: card title "Accounting Break-Even" → "Breaking even" with the sub-line "The sales you need to cover your costs"; card label `true_breakeven` → "What you really need to bring in (loan repayments, tax, drawings)". Break-even, contribution margin and the classification resolver are untouched. `bunx tsgo` clean; `bun test src tests` 213 pass / 0 fail; `bun run security:check` 122 pass plus 18 live access tests, 0 failed, 0 inconclusive, known-failure list unchanged.

- 20 Sep 2026 — CLOSED: two cards nobody could ever see are gone from the catalogue, and the build now refuses to offer a card that has no component. Classification: SECURITY-RELEVANT (touches `app_private.card_group_cards`, which `app_private.client_available_cards` and `app_private.client_cards_v2` read, and rewrites `public.client_cards`). No policy, grant, role, entitlement, purchase, trial or authorisation rule changed; `card_group_cards` stays `immutable`, non-definer, with `execute` revoked from `anon` re-issued after the replace; the resolution path is unchanged (one client-owned ticked list intersected with the purchase). Defect: `bank_reconciliation` (Standard group) and `tax_liability` (Advisory group) were offerable and tickable while no component, import or route existed for either — the third instance in one day of the app accepting a choice and silently ignoring it, after the Fixed cost-of-sales tag the break-even ignored and the pay-run vintage mismatch. Capability was never lost: Tax Liability was superseded by the GST activity statement (`gst_reconciliation`) and PAYG Withholding (`payg_withholding`) cards, Bank Reconciliation by Uncoded Bankfeed Questions (`unreconciled`). Change: one audited migration removed both keys from the catalogue and from every ticked list (14 clients, 24 ticks, one `card_catalogue_cards_removed` audit row recording the counts and reason), and stripped them from `public.org_card_defaults` (no rows held them). Legacy rollback rows are deliberately untouched: `plan_levels` (2), `tier_widget_config` (3), `firms.default_widgets` (1) still name the old keys and are inert — nothing reads them for card resolution. Nothing anyone sees changed: neither key was in `ALL_WIDGETS`, so neither had ever drawn a card. Ticked counts before → after: Positive Traction 17 → 15; the nine DRTABT Projects clients 18 → 16 each; Autotek NSW, Bangkok on King and both ZZ security-test clients 7 → 6; rendered cards per client identical before and after (15, 16, 6 respectively). Guard added: `tests/fixtures/card-catalogue.sql` (read-only snapshot of the catalogue body, dumped by `scripts/dump-card-catalogue.sh` from `pg_proc` because the dumping role has no `app_private` USAGE and should not be given any), `scripts/check-card-catalogue.sh` proving the snapshot is not stale (now first-class in `bun run security:check`), and `tests/card-catalogue.test.ts` which FAILS the build when the catalogue offers a card with no `widgets.includes(...)` branch in the dashboard route or missing from `ALL_WIDGETS`, and WARNS when a built card is offered by no group, so a finished card cannot sit unofferable. Verified the guard fails on a deliberately re-added `tax_liability` entry. Also removed `src/components/dashboard/TrueBreakevenSection.tsx`: `true_breakeven` remains a real, offered, built card whose tick draws the merged Break-Even card through `DEPRECATED_WIDGET_ALIASES`, and that standalone file had no caller. `bun test src` 77 pass / 0 fail; `vitest run tests` 116 pass; `bunx tsgo` clean; `bun run security:check` green with the known-failure list unchanged.
+
+- 20 Sep 2026 — CLOSED: `org_card_defaults` had RLS and a caller-scoped signed-in read policy but was the only public data table missing the standard restrictive `mfa_aal2_required` policy. Added the same `FOR ALL TO authenticated` guard used elsewhere; no grant, permissive policy, card template or row changed. Root control added rather than relying on the after-the-fact posture card: `scripts/check-table-security.ts`, now an early step in `bun run security:check`, reads the fingerprinted live catalogue fixture and fails if any new in-scope public table lacks the exact restrictive aal2 guard, if `anon` receives an unapproved data privilege, or if `authenticated` receives a privilege with no matching permissive policy. The only MFA exclusions are explicit and reasoned: `plan_levels`, `tier_settings`, and recursion-sensitive `session_activity`. Regression tests deliberately remove the organisation-default guard and add DELETE to `security_test_runs`; both are rejected. Exact grants pinned: `org_card_defaults` authenticated SELECT only; `user_presence` authenticated SELECT/INSERT/UPDATE only; `security_test_runs` authenticated SELECT only; all three have no anon privilege. Live catalogue after remediation: 63 public tables, RLS on all, 60 aal2 guards, exactly the three documented exclusions, zero anon table grants.
...
- **Live access-test 429 classification hardened, 17 Sep 2026.** The observed 429 was the app-wide trigger guard (`security_access_tests:global`: six attempts per fixed UTC-aligned 3,600-second bucket), not Supabase Auth; the live bucket had reached 18 while auth analytics showed no 429. A normal run performs four password sign-ins (owner aal2, owner aal1, staff aal2, viewer aal2), with up to three extra only during first-time TOTP enrolment; there are no retries and sessions are reused across all probes. The runner now claims the existing database run-state row to prevent cross-instance overlap, spaces sign-ins one second apart, and classifies trigger/Auth/MFA/server-function 429s as `INCONCLUSIVE — run did not complete`. CLI exit code 2 and the admin warning prevent incomplete runs from appearing green. Supabase's documented default sign-in-related limit is 30 requests per five minutes per IP (burst 30), but the project override was not independently read; the app trigger bucket is global rather than IP-based and cannot consume an office user's Auth allowance.
...
- 17 Sep 2026 — CLOSED: the cost-classification panel now exposes Unclassified as a fourth display/data-entry state and sorts it first. Selecting it uses the existing aal2, caller-scoped delete function, removes the stored row (including its wages marker), and never stores `unclassified`; the unchanged resolver still treats absence as fixed in break-even and cash-flow calculations. Provenance now distinguishes Chosen by hand, Seeded from Xero, and No classification saved · treated as fixed. A permanent matrix probe proves an organisation owner can delete only through the existing row policy and that no `unclassified` value remains. No policy, grant, function signature, schema, resolver precedence or calculation changed. Linter finding classes remained the established RLS-with-no-policy and signed-in-callable definer findings; no new class was introduced. `public.security_posture()` remained unavailable to the sandbox query role as documented in the deferred live-suite item.

- 18 Sep 2026 — CLOSED: break-even disclosure, presentation-only. Classification: not security-relevant — no policy, grant, definer function, role, entitlement, server function or Xero credential touched, and no data added to any response (`unclassifiedCount` and the expense lines were already returned to every authorised viewer of this card). The unclassified-costs warning in `BreakevenWidget.tsx` and the shared `UnclassifiedNotice.tsx` is now shown to any authorised viewer, with the `/clients/$clientId/settings` cost-classification link still rendered only for advisors; the excluded-accounts note was already role-agnostic and is unchanged. The previously silent balancing plug (`opex - linesTotal`) is now exposed as `unitemisedBalance` and listed in the Fixed Costs breakdown as "Unitemised balance", with a prominent warning above 5% of fixed costs. Resolver precedence, the fixed fallback and the monthly-basis units work are untouched. `bun test src` 61 pass / 0 fail; `bun run security:check` 108 tests passed, 18 live access tests passed, 0 failed, 0 inconclusive. Measured across the latest stored profit and loss for all 12 Xero files: the plug is 0.00 in every file (0.00%), so no reading defect is being masked today.

- 18 Sep 2026 — CLOSED: payroll liability cards no longer invent an older period from a reconciliation failure. Classification: SECURITY-RELEVANT (Xero API read path). No policy, grant, definer function, role, entitlement or credential touched; `getPaygWithholdingPosition` / `getSuperannuationPosition` keep `requireAal2` + `assertWidgetAccess`, still read pay runs through `context.supabase` (tenantId a filter, never a grant), and make no additional Xero calls — the account `Type`/`Code` used for the misfiled-account note comes from the `Accounts` payload already fetched for `extractTaxLines`. Cause of the phantom July liability: the newest-first walk added a month, then bailed out on overshoot while leaving `oldestOwingMonth` set to the month it had reached, comparing a LIVE balance against a SAVED pay-run list of an earlier vintage. Fix: new pure `src/lib/xero/payg-reconciliation.ts` (`reconcileBalanceAgainstPeriods`) takes only whole periods that fit inside the balance, reports the remainder as an explicit `residue` with a `residueKind` decided by vintage (`since_last_pay_run` when the balance was read after the saved runs, `unmatched` when both sides share a vintage), and can never name a period it merely reached. Pay runs dated after the saved list's own `as_at` are excluded from the monthly/payday figures instead of being absorbed. `loadPayRuns` now returns the stored row's own `SnapshotSource`, so the truncated (`complete = false`) pay-run pull is reported as incomplete instead of being rebuilt by hand as `complete: true, stale: false` — that hand-built source was suppressing both flags on both cards. Both cards show the two vintages side by side whenever they differ, and the PAYG card flags PAYG accounts filed outside current liabilities (Positive Traction: PAYG Withholding Payable 21420 under Non-current Liability). New `src/lib/xero/payg-reconciliation.test.ts` pins the case that broke: a live balance of 4,114 against saved months of 1,172 + 2,348 + 2,937 yields owing = September, August, residue 594 `since_last_pay_run`, and July is never named. OPEN, no change made: `src/lib/xero/gst.server.ts` mixes live GST report figures with saved pay runs for the BAS period and marks the estimate incomplete only on a read failure, not when the saved pay-run list is older than the period end or was truncated — same defect class, needs an owner decision because it changes a reported BAS completeness flag.

- 18 Sep 2026 — CLOSED: BAS/GST estimate no longer combines a live figure with a saved one silently. Classification: SECURITY-RELEVANT (Xero API read path). No policy, grant, definer function, role, entitlement or credential touched; `computeGstReconciliation` still reads pay runs through the caller's `supabase` via `loadPayRuns`, tenant as a filter, and makes no additional Xero calls. `GstResult` gains `vintages` (gstReadAt, payRunsFromSnapshot, payRunsAsAt, payRunsFetchedAt, payRunsComplete, payRunsCoverPeriodEnd, latestPayRunDate, differs), all taken from the stored row's OWN `SnapshotSource` and never rebuilt as complete/fresh. Pay runs dated after the saved list's as-at are excluded from the period total rather than absorbed, and nothing is estimated in their place. New shared rule `payRunVintageIssues` in `src/lib/xero/payg-reconciliation.ts` makes the estimate report itself incomplete when the saved list stops before the period end OR was a partial pull, not only when a read fails; four tests cover both conditions, the both-fit case and the live-read case. `GstReconciliationWidget.tsx` carries a prominent "Estimate only — check against Xero before lodging" line naming the vintage of each input, plus explicit notes for the stale and partial cases. Audit of every source construction in `src`: the only places building a `SnapshotSource` by hand are `liveSource`/`snapshotSource` helpers in `snapshot-source.ts`; `snapshot-read.server.ts` is the sole reader and passes the stored flags through. No remaining card rebuilds a stored copy's flags as complete and fresh.

- 18 Sep 2026 — CLOSED: cost-of-sales lines are now classified. Classification: not security-relevant — presentation/calculation only. No policy, grant, definer function, role, entitlement, server function signature or Xero credential touched; no field added to any response (`cogsLines` was already returned by `getProfitAndLoss` and is already read by the classification panel), and no additional Xero call. Defect: `useBreakevenData` resolved only `expenseLines` and added `totalCostOfSales` to variable costs as an unclassified lump, so an account tagged Fixed in the Cost of Sales section (Positive Traction: Wages & Salaries, Superannuation) was displayed as Fixed on the classification panel and silently treated as variable by Accounting Break-Even. Fix: new pure `src/components/dashboard/breakeven-lines.ts` (`classifyBreakevenLines`) resolves BOTH P&L sections through the unchanged resolver; the only difference is the fallback for a line nothing has decided — operating expenses keep the fixed fallback and the unclassified count, cost of sales falls back to variable, which is what every client saw before this change. Resolver precedence, the fixed fallback for operating expenses, the wages marker and the monthly-basis units work are untouched. The breakdown now lists Fixed Costs (cost-of-sales lines marked), Variable Opex and Cost of Sales, variable as separate groups, each reconciling to Xero's reported totals with its own visible unitemised balance. The cash-flow scenario's three-month reference average and its month breakdown now group classification-first (a Fixed-tagged cost-of-sales line appears under Fixed, not Cost of sales), so the two cards on the same dashboard agree. New `src/components/dashboard/breakeven-lines.test.ts` pins the missing assertion: a cost-of-sales account tagged Fixed lands in fixed costs; plus untagged-stays-variable, untyped-untagged-stays-variable, excluded, plug and classification-disabled cases. Scope check across all stored classifications: only Positive Traction holds any `fixed`/`excluded` tag; the only other tagged client (Autotek New South Wales) has one `variable` tag, so the other 11 files are arithmetically unchanged. `bunx tsgo` clean; `bun test src` 77 pass / 0 fail; `bun run security:check` 108 passed, 18 live access tests, 0 failed, 0 inconclusive.
- 18 Sep 2026 — OPEN (separate finding, no change made): business health "months of runway" divides available cash by operating expenses only and excludes cost of sales, so for a client whose wages sit in cost of sales (Positive Traction: ~$9,985 of $10,474 monthly costs) runway is materially overstated. Needs an owner decision on the divisor before any change.

- 20 Sep 2026 — CLOSED: two cards nobody could ever see are gone from the catalogue, and the build now refuses to offer a card that has no component. Classification: SECURITY-RELEVANT (touches `app_private.card_group_cards`, which `app_private.client_available_cards` and `app_private.client_cards_v2` read, and rewrites `public.client_cards`). No policy, grant, role, entitlement, purchase, trial or authorisation rule changed; `card_group_cards` stays `immutable`, non-definer, with `execute` revoked from `anon` re-issued after the replace; the resolution path is unchanged (one client-owned ticked list intersected with the purchase). Defect: `bank_reconciliation` (Standard group) and `tax_liability` (Advisory group) were offerable and tickable while no component, import or route existed for either — the third instance in one day of the app accepting a choice and silently ignoring it, after the Fixed cost-of-sales tag the break-even ignored and the pay-run vintage mismatch. Capability was never lost: Tax Liability was superseded by the GST activity statement (`gst_reconciliation`) and PAYG Withholding (`payg_withholding`) cards, Bank Reconciliation by Uncoded Bankfeed Questions (`unreconciled`). Change: one audited migration removed both keys from the catalogue and from every ticked list (14 clients, 24 ticks, one `card_catalogue_cards_removed` audit row recording the counts and reason), and stripped them from `public.org_card_defaults` (no rows held them). Legacy rollback rows are deliberately untouched: `plan_levels` (2), `tier_widget_config` (3), `firms.default_widgets` (1) still name the old keys and are inert — nothing reads them for card resolution. Nothing anyone sees changed: neither key was in `ALL_WIDGETS`, so neither had ever drawn a card. Ticked counts before → after: Positive Traction 17 → 15; the nine DRTABT Projects clients 18 → 16 each; Autotek NSW, Bangkok on King and both ZZ security-test clients 7 → 6; rendered cards per client identical before and after (15, 16, 6 respectively). Guard added: `tests/fixtures/card-catalogue.sql` (read-only snapshot of the catalogue body, dumped by `scripts/dump-card-catalogue.sh` from `pg_proc` because the dumping role has no `app_private` USAGE and should not be given any), `scripts/check-card-catalogue.sh` proving the snapshot is not stale (now first-class in `bun run security:check`), and `tests/card-catalogue.test.ts` which FAILS the build when the catalogue offers a card with no `widgets.includes(...)` branch in the dashboard route or missing from `ALL_WIDGETS`, and WARNS when a built card is offered by no group, so a finished card cannot sit unofferable. Verified the guard fails on a deliberately re-added `tax_liability` entry. Also removed `src/components/dashboard/TrueBreakevenSection.tsx`: `true_breakeven` remains a real, offered, built card whose tick draws the merged Break-Even card through `DEPRECATED_WIDGET_ALIASES`, and that standalone file had no caller. `bun test src` 77 pass / 0 fail; `vitest run tests` 116 pass; `bunx tsgo` clean; `bun run security:check` green with the known-failure list unchanged.

- 20 Sep 2026 — CLOSED: `org_card_defaults` had RLS and a caller-scoped signed-in read policy but was the only public data table missing the standard restrictive `mfa_aal2_required` policy. Added the same `FOR ALL TO authenticated` guard used elsewhere; no grant, permissive policy, card template or row changed. Root control added rather than relying on the after-the-fact posture card: `scripts/check-table-security.ts`, now an early step in `bun run security:check`, reads the fingerprinted live catalogue fixture and fails if any new in-scope public table lacks the exact restrictive aal2 guard, if `anon` receives an unapproved data privilege, or if `authenticated` receives a privilege with no matching permissive policy. The only MFA exclusions are explicit and reasoned: `plan_levels`, `tier_settings`, and recursion-sensitive `session_activity`. Regression tests deliberately remove the organisation-default guard and add DELETE to `security_test_runs`; both are rejected. Exact grants pinned: `org_card_defaults` authenticated SELECT only; `user_presence` authenticated SELECT/INSERT/UPDATE only; `security_test_runs` authenticated SELECT only. Live catalogue after remediation: 63 public tables, RLS on all, 60 aal2 guards, exactly the three documented exclusions, zero anon table grants.

- 20 Sep 2026 — CLOSED: per-file hourly ceiling on outbound Xero calls. Classification: SECURITY-RELEVANT (Xero API read path). No policy, grant, definer function, role, entitlement, membership or credential touched; no new access path and no caller input trusted — the key is the server-resolved `tenant_id` of an already-authorised connection. Cause of the 20 Sep 01:00 UTC burst on Positive Traction (301 calls, 24 per-minute rejections at 01:36, minute allowance 0/60, day allowance 4,722/5,000 remaining): concentrated diagnostic work on ONE file inside eleven minutes — a `view_as_started` session at 01:25, a manual snapshot refresh at 01:26 (12 reports, `xero_snapshot_manual` bucket = 1), then repeated dashboard loads across several date ranges while the cost-of-sales defect was traced (158 `xero_data_read` audit rows 01:26–01:36, peaking at 38 in the 01:35 minute). Not a retry loop: `xero_api_errors` has no row in that hour, the 429s carried `X-Rate-Limit-Problem: minute` with `Retry-After: 7` and were retried on the bounded rate budget (3 attempts, `waitForRateLimit` honouring `Retry-After`, capped at 60s), a `day` rejection still stops immediately, and 401/403 behaviour is unchanged. Eight `xero_token_refreshed` rows within four seconds at 01:26:03–06 are concurrent fan-out on an expiring token, not repetition. Not the break-even/cost-of-sales work either: the new modules are pure, and `useBreakevenData`'s three query keys are stable primitives (`["xero-pnl", tenantId, from, to, basis]`, `["cost-classifications", clientId, tenantId]`, `["xero-expense-accounts", clientId, tenantId]`) with no fetch in render and no unstable dependency. Fix: new `src/lib/xero/file-ceiling.server.ts` counts every outbound accounting, assets and payroll request against `public.check_rate_limit` under `xero_file_hour:<tenant_id>`, fixed UTC-aligned hour, ceiling 200 — about 14x a file's normal daily use, above a full manual refresh, and far below Xero's 5,000/day. The counter lives in Postgres, so it holds across backend instances (the previous throttle was per instance and excluded interactive traffic). Over the ceiling the request is refused with wording the existing `isXeroRateLimitMessage` predicate already recognises, so cards show a calm pause and saved figures. Fails open only if the limiter itself errors, so a broken counter cannot black out client data; Xero's own limits remain the backstop. Admin-client register updated. New `tests/xero-file-ceiling.test.ts` (5 tests) pins the key, window, refusal, fail-open and predicate match. No client lost access: 278 of 5,000 daily calls used, no connection marked disconnected, no data loss.

- 20 Sep 2026 — CLOSED: break-even render race and margin naming. Classification: not security-relevant — presentation only. No policy, grant, definer function, role, entitlement, server function signature or Xero credential touched; no calculation changed. Defect: `useBreakevenData` rendered as soon as the profit and loss resolved, so an in-flight cost-classification list read as an empty list and both of Positive Traction's Fixed-tagged cost-of-sales accounts (Wages & Salaries, Superannuation) seeded as variable, showing a confident $896/month fixed cost and 19.3% margin. Fix: new pure `src/components/dashboard/breakeven-readiness.ts` gates calculation until the report, the stored classifications and the account list have all resolved or failed; `figures` is null until then; a failed classification read now says so on the card with a Try again action instead of falling back to defaults. New `breakeven-readiness.test.ts` (8 cases) pins the missing assertion — report loaded with classifications still pending produces no figures — and a static guard in `tests/static-guards.test.ts` stops the card being rewired to calculate from partial inputs. Other classification consumers checked: the cash-flow scenario, its three-month reference breakdown and business health all resolve classifications server-side inside a single server function, so they cannot render half-loaded; the classification panel already waited. Naming: the break-even row, formula line and breakdown now read "Contribution Margin % (after variable costs only — not the same as gross margin in Xero)". Business Health's section-based gross margin is unchanged and is the only remaining "gross margin" label. Positive Traction 1–20 Sep: fixed $896 → $15,873/month, margin 19.3% → 79.1%, break-even ~$4,600 → $20,063/month, revenue $25,030 unchanged, above break-even by ~$4,967. `bunx tsgo` clean; `bun test src tests` 207 pass / 0 fail.
