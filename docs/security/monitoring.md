...
- **Live access-test limits (verified 17 Sep 2026)**: the observed 429 came from the app's `security_access_tests:global` trigger bucket (six attempts per fixed UTC-aligned hour, app-wide), not Supabase Auth; auth analytics contained no corresponding 429. The steady-state suite makes four password sign-ins and first-time TOTP setup can make seven, with no retries; sessions are reused across all assertions. Supabase documents a default sign-in-related allowance of 30 requests per five minutes per IP (burst 30), so office users could only share an Auth allowance if traffic actually reached Auth from the same public IP. The observed app bucket cannot consume that allowance or lock out office users. Runs are cross-instance serialised, sign-ins are one second apart, and any trigger, Auth, MFA or probe 429 is recorded and printed as incomplete/inconclusive rather than pass or failure.
...
- **Concurrent-limit 429s are expected and self-resolving** (recorded 16 Sep 2026, owner decision: monitor only). Xero enforces a third limit besides the day and minute quotas: at most 5 requests in flight at once per Xero file, reported as `X-Rate-Limit-Problem: concurrent`. First observed 16 Sep 2026 on the Positive Traction file — 53 calls in the 02:00 UTC hour (10:00 Perth), 4 refused at 02:44 UTC while a scheduled refresh overlapped interactive traffic; all four honoured `Retry-After`, retried and completed. The snapshot-refresh worker caps itself at 2 in-flight calls, but that gate is per backend instance and does not count interactive traffic, so the ceiling can still be crossed — this is by design until occurrences repeat. The badge counts retries, not failures: no request failed and no data was lost. Worry only if concurrent rejections repeat within one day, counts climb, or any `day`-problem 429 appears; a cross-instance throttle is the planned remedy and is not built.
