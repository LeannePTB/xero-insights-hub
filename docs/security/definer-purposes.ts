...
export const DEFINER_PURPOSES: Record<string, string> = {
  ...
  "app_private.firm_branding_enabled":
    "Answers whether one organisation's Branding option is effective right now — purchased OR an unexpired trial that explicitly includes Branding, and only while the organisation is not lapsed. Execute revoked from PUBLIC, anon and authenticated: it is only reachable from public.client_branding_enabled, which authorises the caller first. Holds no authorisation of its own and returns no client, organisation or personal data.",
  "public.client_branding_enabled":
    "Answers whether one client may carry its own logo on the monthly management report. aal2 first, then refuses unless the caller can already read that client (public.user_can_read_client) — the client id is a filter, never a grant — then returns app_private.firm_branding_enabled for that client's organisation. An entitlement answer only: it can narrow what an authorised caller may do and can never widen it, and it returns no client data.",
