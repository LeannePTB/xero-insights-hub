...
## 0b. Daily 3am sign-in cut-off (14 Sep 2026)

Every signed-in person must sign in again after 3am **Australia/Sydney** each day; AEST/AEDT is handled by the tz database. The cut-off is the most recent 3am that has **already passed**: between midnight and 3am Sydney local time that is *yesterday's* 3am, not today's (14 Sep 2026 fix — using today's date unconditionally put the cut-off in the future during those three hours, so every session, including one seconds old, was stale and every data table returned nothing). Enforcement is server-side and layered; the browser layers can only deny earlier, never grant.

- **Database (the enforcement point).** `app_private.is_session_fresh()` (SECURITY DEFINER, `SET search_path`, registered) reads the `session_id` claim, looks up `auth.sessions.created_at` and returns whether it is at or after the cut-off. No request context and `service_role` are system contexts and pass. It **fails closed**: a missing `session_id` claim or a session row it cannot find is stale. `app_private.is_aal2()` now requires it in addition to the `aal` claim, so the RESTRICTIVE `mfa_aal2_required` policy hides every row on every data table from a stale session. `app_private.assert_aal2()` checks freshness first and raises `SESSION_EXPIRED` (`insufficient_privilege`) before the MFA check, so callable definer functions and server functions report the accurate reason. There is no public wrapper: `public.session_fresh()` was dropped on 14 Sep 2026 (nothing called it, and being unable to reference the aal2 guard it could only ever be a standing exception in the posture check).
- **Server-function middleware (Layer 3, deny-only).** `dailySignInMiddleware` in `src/start.ts` rejects a request whose bearer token shows a session start before the cut-off with a generic 401. It decodes the token payload **without verifying the signature** — deliberate, because this layer can only deny and is never the sole control. Session start is the minimum `amr[].timestamp` (which survives hourly token refresh), falling back to `iat` when `amr` is absent; `iat` alone is not a freshness signal.
