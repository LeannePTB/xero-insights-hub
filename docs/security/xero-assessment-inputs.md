...
| How long can a session live? | Spec § 0b and § 0c: a daily 3am Australia/Sydney sign-in cut-off and a 30 minute inactivity timeout, both enforced in the database on every table and every guarded function, with a server-held activity timestamp no caller can set | **Yes** |
