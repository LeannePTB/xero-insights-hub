...
| How long can a session live? | Spec § 0c: a 30 minute inactivity timeout, enforced on the server and in the database on every table and every guarded function, with a server-held activity timestamp no caller can set, plus sign-out on demand. A daily 3am sign-in cut-off existed 14–15 Sep 2026 and was removed by owner decision (spec § 0b) because the inactivity timeout covers the stolen-device threat directly | **Yes** |
| Can a person end a session on a lost device? | Settings → Account, "Sign out my other devices" (`scope: "others"`, revoked in the authentication service, audited). Signing *another* person out remotely is not available on this platform — backlog 50 | **Partial** |
