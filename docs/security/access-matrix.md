...
| 28 | Support-grant holder, active, non-member organisation | client_subscriptions | insert | Phase 3a dropped 'staff manage client subscriptions' (the support-grant write policy). Still admitted by 'super admins manage client subscriptions', because every support grantee is a super admin, and no audit row is written. Fixed in Phase 3b. |
| 28 | Support-grant holder, active, non-member organisation | client_subscriptions | update | Phase 3a dropped 'staff manage client subscriptions' (the support-grant write policy). Still admitted by 'super admins manage client subscriptions', because every support grantee is a super admin, and no audit row is written. Fixed in Phase 3b. |
| 28 | Support-grant holder, active, non-member organisation | client_subscriptions | delete | Phase 3a dropped 'staff manage client subscriptions' (the support-grant write policy). Still admitted by 'super admins manage client subscriptions', because every support grantee is a super admin, and no audit row is written. Fixed in Phase 3b. |
...
| 29 | Super admin with NO membership | signup_requests | update | 'super_admin updates signup_requests' is on is_super_admin alone and writes no audit row (re-targeted from role public to authenticated on 11 Sep 2026). |
...
| signup_requests | update | DENY **KNOWN FAILURE (backlog 29)** | pglite, live | Spec §9 (platform metadata changes must leave an audit row) | 'super_admin updates signup_requests' is on is_super_admin alone and writes no audit row (re-targeted from role public to authenticated on 11 Sep 2026). |
...
| client_subscriptions | insert | DENY **KNOWN FAILURE (backlog 28)** | pglite, live | PK 5 (support grants are READ-ONLY); Spec §8 (a comp needs a reason and an audit row) | Phase 3a dropped 'staff manage client subscriptions' (the support-grant write policy). Still admitted by 'super admins manage client subscriptions', because every support grantee is a super admin, and no audit row is written. Fixed in Phase 3b. |
| client_subscriptions | update | DENY **KNOWN FAILURE (backlog 28)** | pglite, live | PK 5 (support grants are READ-ONLY); Spec §8 (a comp needs a reason and an audit row) | Phase 3a dropped 'staff manage client subscriptions' (the support-grant write policy). Still admitted by 'super admins manage client subscriptions', because every support grantee is a super admin, and no audit row is written. Fixed in Phase 3b. |
| client_subscriptions | delete | DENY **KNOWN FAILURE (backlog 28)** | pglite, live | PK 5 (support grants are READ-ONLY); Spec §8 (a comp needs a reason and an audit row) | Phase 3a dropped 'staff manage client subscriptions' (the support-grant write policy). Still admitted by 'super admins manage client subscriptions', because every support grantee is a super admin, and no audit row is written. Fixed in Phase 3b. |
| xero_connections.access_token_enc | read | DENY | pglite, live | PK 8; Spec §10 (no column grant; privilege check precedes RLS) |  |
| xero_connections.refresh_token_enc | read | DENY | pglite, live | PK 8; Spec §10 (no column grant; privilege check precedes RLS) |  |
| audit_log | insert | DENY | pglite, live | PK 10; Spec §9 (append-only) |  |
| audit_log | update | DENY | pglite, live | PK 10; Spec §9 (append-only) |  |
