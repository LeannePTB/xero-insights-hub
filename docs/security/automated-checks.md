...
| `xero_rate_limits` | `public.xero_rate_limit_posture()` reads `public.xero_rate_limits`, which holds the remaining quota Xero itself reports on every response (`X-DayLimit-Remaining`, `X-MinLimit-Remaining`, `X-AppMinLimit-Remaining`) | **Warn** when any Xero file's lowest remaining figure is under 20% of the published allowance. **Action** under 5%, on any 429 rate-limit rejection in the last 24 hours, or when one file exceeded 300 calls in a single hour (burst — a runaway retry loop reaches this long before the daily cap). Every figure is computed from the table; only Xero's published caps (5,000/day, 60/minute per file, 10,000/minute app-wide) are constants, used solely to express a remaining count as a percentage |
