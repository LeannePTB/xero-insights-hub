...
TypeScript posture checks in `src/lib/security-posture.functions.ts`, which are
only the ones the database cannot read for itself — a TLS handshake, a header
read, an encryption round trip).

Every database-side check is returned by `public.security_posture()` itself, and
`tests/static-guards.test.ts` §12e fails the build if a `*_posture()` function
exists in `public` that `security_posture()` does not call. Three checks had
been built and left unwired (`test_accounts`, `read_audit`, `xero_rate_limits`);
the last of those was invisible to the owner until 16 Sep 2026. A check nobody
...
| `session_controls` | `public.session_controls_posture()` reads the live source of `app_private.is_session_active`, `is_aal2` and `assert_aal2`, and counts live sessions with no activity record | **Action** when the inactivity window cannot be read, when the aal2 gate no longer consults the idle check, when `assert_aal2` no longer raises `SESSION_IDLE`, when `touch_session_activity` is not a definer function, or when the removed daily sign-in cut-off is only half removed (any surviving `is_session_fresh` reference or the function itself). **Warn** (added 15 Sep 2026) when any live session is past the window with no `session_activity` row — the exact condition behind the 15 Sep lockouts: expected for one or two sessions after a release, a rising count means activity is not being recorded at all. The daily cut-off was removed on 15 Sep 2026 by owner decision, so the check reports the inactivity window only and never claims a daily control |
| `xero_rate_limits` | `public.xero_rate_limit_posture()` reads `public.xero_rate_limits`, which holds the remaining quota Xero itself reports on every response (`X-DayLimit-Remaining`, `X-MinLimit-Remaining`, `X-AppMinLimit-Remaining`) | **Warn** when any Xero file's lowest remaining figure is under 20% of the published allowance. **Action** under 5%, on any 429 rate-limit rejection in the last 24 hours, or when one file exceeded 300 calls in a single hour (burst — a runaway retry loop reaches this long before the daily cap). Wired into `public.security_posture()` on 16 Sep 2026; it had been merged only by the app, where an RPC failure was swallowed, so nothing appeared on the Security page. The window is the most recent day with usage, today or yesterday (UTC) — the previous "today only" window reported nothing between UTC midnight and the nightly refresh at ~17:05 UTC. Every figure is computed from the table; only Xero's published caps (5,000/day, 60/minute per file, 10,000/minute app-wide) are constants, used solely to express a remaining count as a percentage |
