...
declare
  ...
  -- Window: today and yesterday (UTC). The nightly refresh runs at ~17:05 UTC,
  -- so a "today only" window reported nothing at all between UTC midnight and
  -- the next run — the check went silent for the busiest part of the day.
...
begin
  ...
  select max(day) into latest_day
  from public.xero_rate_limits where day >= win_from;
