-- Phase 2 part B — close the firms ownership escalation (backlog 27).

-- 1. Record the practice organisation as a fixed platform setting.
create table if not exists app_private.platform_settings (
...
values ('practice_firm_id', '4dcfd606-dce3-4674-923f-c5183ecae141')
...
$fn$;
revoke execute on function app_private.practice_firm_id() from public, anon;

-- 2. No browser session writes public.firms. Every legitimate path runs as
--    service_role or through a SECURITY DEFINER function.
revoke insert, update, delete, truncate, references, trigger
  on table public.firms from anon, authenticated;
revoke all on table public.firms from anon;
grant select on table public.firms to authenticated;

-- 3. A bare super admin may read organisations (Path C) but never update them.
drop policy if exists "super_admin updates firms" on public.firms;

-- Re-target the remaining policies from role public to authenticated (Spec §6).
...
create policy "firm members read own firm" on public.firms
  ...
  using (app_private.has_firm_access(auth.uid(), id));
...
-- Kept deliberately (owner decision, 11 Sep 2026): unreachable without an
-- UPDATE grant; removing it is a Phase 7 change.
...
create policy "firm owners update own firm" on public.firms
  ...
  using (app_private.is_firm_owner(auth.uid(), id))
...
-- 4. The only way to change the always-free flag: aal2 + super admin, a reason,
--    an audit row, and TRUE only on the practice organisation.
...
begin
  ...
  if _value and _firm_id <> app_private.practice_firm_id() then
...
$fn$;
revoke execute on function public.set_firm_always_free(uuid, boolean, text) from public, anon;
grant execute on function public.set_firm_always_free(uuid, boolean, text) to authenticated;

-- 5. Posture: the always-free flag anywhere but the practice organisation is an Action.
