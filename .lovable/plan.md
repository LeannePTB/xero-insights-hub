...
- Organisation mapping from the design doc: DRTABT Projects (9 clients, Advisory on, Consolidation on, bookkeeping); Positive Traction (1, Advisory on, Consolidation n/a — single client, bookkeeping); Bangkok On Darby and Autotek NSW (1 each, Advisory off, bookkeeping). ZZ Security Test Org backfilled like any other so the access matrix keeps working.
- `loan_consolidation` is a Consolidation card, so it lands only on DRTABT's nine lists. Positive Traction keeps that card today without Consolidation — an owner decision is flagged below.
...
2. `client_allowed_widgets` gains a switch (`app_private.card_model_v2` boolean): false = old resolution, true = `client_visible_cards`. Reads flip by setting one boolean.
3. **Switch back in one step:** set it false. Old tables are untouched and still current.
...
- New matrix rows: an organisation without Advisory, and one without Consolidation, refused a card from that group via dashboard read, direct URL, server function and a saved report link.
